...
- **Settings** — Admin-only Settings view (sidebar gear icon) for configuring lab-wide options. Currently: lab profile switcher (`plant_tissue_culture | cell_culture | mycology`) with a warning banner and mandatory `CHANGE PROFILE` confirmation when data already exists; profile change updates the Svelte store immediately so all components react without restart. Both `plant_tissue_culture` and `cell_culture` profiles have fully seeded vocabulary (stages, propagation methods, hormone types, compliance types, agencies, and inventory categories); `mycology` vocabulary is not yet seeded (v1.14.0, v1.15.0).
...
- **Taxonomy Navigator** *(v1.17.0)* — Two-column browser: Species list with live search → Strains for selected species with status badges and specimen counts → Slide-in panel showing specimens bound to a strain. Status filter: All | Unverified | Claimed | Confirmed (Manual) | Confirmed (Genomic) | Confirmed (Any). Accessible via "Taxonomy" sidebar entry. Phase TX-2 expands to a full hierarchical browser with advanced filtering.
...
- **Automated Tests** — Vitest test suite covers utility functions, export row builders, import validation, and lab profile store — ~107 assertions across 4 test files. Rust unit tests (136 test functions) cover accession number generation, pagination helpers, stock depletion rules, all four compliance auto-flag SQL rules, **hash-chain invariants** (per-lineage `chain_seq`, fork invariants, `compute_entry_hash` determinism), Merkle checkpoint correctness, death workflow invariants, profile-aware dashboard queries, check_profile_change_allowed confirmation logic, cell_culture vocabulary seeding, **strain/cultivar data model** invariants (status transitions, hash-chain seeding, hybridization guard), **taxonomy backbone** (taxa table structure, rank constraint, backfill idempotency), and **pedigree traversal** (ancestry, descendants, max-depth capping, cycle detection in both directions, specimen tree with/without descendants, export bundle). Both `npm test` and `cargo test` run in CI on every push and block merges on failure (v1.2.4, v1.4.1, v1.6.0, v1.11.0, v1.13.0, v1.14.0, v1.15.0, v1.16.0, v1.17.0, v1.18.0, v1.20.0, v1.21.0).
