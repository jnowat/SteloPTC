...
| **PWA (web)** *(v1.40.0)* | *Not yet hosted — build locally with `npm run build`* | Installable manifest + service worker are produced in `dist/`; no CI workflow deploys this build to a public URL yet. See [PWA status](#progressive-web-app-pwa-status-v1400-wp-62) below for what does/doesn't work without the desktop app |
...
- **Camera permission (v1.39.2)** — `src-tauri/Info.ios.plist` supplies `NSCameraUsageDescription`, wired in via `bundle.iOS.infoPlist`, so the QR scanner doesn't crash on camera access. Believed correct per Tauri's documented config-merge behavior, but — like everything else on this list — unverified against a real Xcode build.
...
### Progressive Web App (PWA) status (v1.40.0, WP-62)

`npm run build` produces both the Tauri desktop bundle input and an installable PWA (manifest + service worker via `vite-plugin-pwa`) from the same `dist/` output. The service worker is gated at runtime to **never** register inside the Tauri desktop webview, so it cannot interfere with the desktop app's `ipc://` calls under any circumstance.

**Works the same in the PWA as on desktop:** all read views (Dashboard, Specimen list/detail, Analytics, Audit Log, etc.) once cached; installable via the browser's "Add to Home Screen"/install prompt.

**Requires the full desktop (Tauri) app — not yet available in a browser-only PWA install:**
- Any data mutation. SteloPTC's entire command layer is Tauri IPC (`invoke()`), which only exists inside the Tauri webview — there is currently no remote HTTP API for a plain browser tab to reach. The offline mutation queue (WP-62) is fully implemented and tested (ordering, replay, failure handling) but has nothing to replay against yet outside Tauri.
- QR camera scanning, native file-system access (attachments, local backup/restore), OS-native print, and desktop notifications.

In short: the PWA today is a read-only, installable shell. Making it fully functional standalone would require a remote API server — a natural next step, not yet built.
...
- **Advanced Hybridization Tools** *(v1.36.0)* — Significantly enhanced hybridization system. Generation labeling (F1/F2/F3/F4, BC notation, custom) with automatic backcross detection: `detect_backcross` walks the `strain_parents` pedigree graph (depth 10, cycle-guarded) to find when one parent is an ancestor of the other; `suggest_generation_label_for_parents` composes backcross detection with filial label rules (backcross takes priority, e.g. depth 1 → `BC1F1`). `create_hybridization_event` stores `generation_label` and `backcross_depth` in `hybridization_events` and sets `is_cross_species = 1` on cross-species strains. Cross-species hybridization is **blocked for normal users** with a clear error; **admin-only override path** requires explicit scientific justification text, writes a permanent non-removable `cross_species_override` audit entry, and sets the `is_cross_species` flag. `HybridWizard` step 5 shows live auto-suggestion (green for filial, amber for backcross) and a quick-select of common labels; step 3 shows the admin override panel with consent checkbox. `StrainDetail` displays a permanent red cross-species warning banner and a generation label badge (blue filial, amber backcross). `suggest_generation_label` and `get_generational_stats` Tauri commands exposed. Test coverage for all generation-label and backcross helpers shipped as part of v1.21.0 (WP-38).
- **Custom Taxa & Darwin Core Export** *(v1.37.0)* — Provisional/lab-internal taxon management with mapping to accepted NCBI taxa and Darwin Core JSON export. Migration 034 adds `status TEXT` (default `'accepted'`) and `provisional_notes TEXT` columns to `taxa`, plus a `taxon_mappings` join table linking provisional → accepted records. Five new Tauri commands: `create_provisional_taxon` (supervisor/admin only), `list_provisional_taxa`, `map_provisional_taxon` (supervisor/admin only), `list_taxon_mappings`, and `export_darwin_core` (accepts optional `root_id` for subtree export via recursive CTE). Darwin Core output uses camelCase field names (`taxonID`, `scientificName`, `taxonRank`, `parentNameUsageID`, `taxonomicStatus`, `nameAccordingTo`, `remarks`) with status mapping (`provisional` → `provisionallyAccepted`). New **Provisional Taxa** sidebar entry opens `ProvisionalTaxaManager` with list/detail layout, per-taxon mapping cards, and a one-click JSON download. 11 new Rust unit tests (5 migration, 6 query). Total: 282 Rust tests.



- **Local AI Analysis** *(v1.40.0, WP-56)* — On-device AI assistance via a local Ollama instance (no cloud calls, ever — a hand-rolled minimal HTTP client talks only to `http://127.0.0.1:11434`). **Summarize Notes**, **Suggest Passage Comment** (from recent passage history), and **Analyze Photo for Contamination** (vision model) each produce a *pending* suggestion (new `ai_suggestions` table) that requires explicit human approval before it ever touches a real `notes` field — approval goes through the same audit-logged update path a manual edit uses. UI buttons live in the specimen notes area and the photo lightbox. All manual note-editing workflows work identically with or without Ollama running.
- **Interactive Lab Map** *(v1.40.0, WP-57)* — Optional floor-plan image + pin coordinates per location (new `locations` table), purely additive to the existing free-text Room/Rack/Shelf/Tray fields. `LabMap.svelte` renders pins over the floor plan with a density/contamination-risk/age heat-map toggle, click-to-manage, and a Dashboard overview widget. No mapping-library dependency was added.
- **Advanced Analytics & Reporting Dashboards** *(v1.40.0, WP-58)* — A dedicated Analytics view: KPI strip (active specimens, passages this week, contamination rate, throughput, growth trend), a global time-range selector (30d/90d/1y/all), trend charts for growth rate/subculture frequency/contamination rate/passage success/media efficiency (hand-rolled inline SVG, no charting-library dependency added), a sortable Strain Performance comparison, a Technician Activity report (supervisor/admin only, framed as workload visibility), and a multi-sheet Excel export.
- **Cloud Backup & Multi-Device Sync (Zero-Knowledge E2E Encryption)** *(v1.40.0, WP-59)* — Encrypted offsite backup: Argon2id-derived AES-256-GCM encryption, passphrase never persisted anywhere. `local_nas`/`smb` targets (a filesystem path, e.g. a mounted network share) are fully functional for backup, restore, and multi-device sync; `s3`/`sftp` targets can be configured today but return a clear "not yet connected" error, since no network client library was added in this pass. Restore uses the same two-step destructive-confirmation flow as the existing local backup restore. Configured from **Settings → Cloud Backup**.
- **Regulatory Compliance Export Modules** *(v1.40.0, WP-60)* — FDA 21 CFR Part 11 signed attestation bundles (Ed25519 signing — chosen over RSA-4096 for a much smaller dependency with the same guarantee), USDA APHIS PPQ Form 526 pre-fill, and CITES Species Provenance Dossiers (chain-of-custody + the existing Darwin Core export), all generated from **Compliance → Regulatory Export** (supervisor/admin only). See [`docs/regulatory-exports.md`](docs/regulatory-exports.md).
- **Plugin / Extension System** *(v1.40.0, WP-61)* — Install `.steloplugin` manifests (JSON) to add a new lab profile with seeded vocabulary, from **Settings → Plugin Manager**. Vocabulary seeding is idempotent, profile-isolated, and never rolled back on uninstall. WASM compliance-rule execution is validated/recorded but not yet executed in this release — see [`docs/plugin-authoring.md`](docs/plugin-authoring.md) for the full format and a worked example.
- **Progressive Web App (PWA) & Offline-First** *(v1.40.0, WP-62)* — The web-export build is now installable (manifest + service worker via `vite-plugin-pwa`) with an IndexedDB-backed offline mutation queue. **Important:** the service worker only ever activates outside the Tauri desktop webview (a runtime check gates registration), so the desktop app is completely unaffected. See "Desktop vs. PWA feature parity" below for what currently does and doesn't work in a browser-only install.
- **Performance & Scalability Hardening (100k+ specimens)** *(v1.40.0, WP-63)* — New covering indexes for the specimen list/filter, passage-timeline, and compliance queries; an in-memory materialized dashboard cache (60s TTL, invalidated on every count-changing write); cursor-based (`chain_seq`) pagination for viewing a lineage's full audit history without loading it all at once; a configurable pedigree traversal depth cap (Settings, 1–20); and hand-rolled DOM virtualization in the Specimens list so rendered row count stays bounded regardless of total results. New Criterion benchmark suite (`benches/`, CI canary, non-blocking).
- **Taxon Chain Re-anchoring Tool** *(v1.40.0, WP-64)* — A supervised `reanchor_taxon_chain` command (admin only, with a dry-run pre-flight report) safely re-derives cryptographic genesis entries for every taxon/species/strain/specimen affected by a taxon reclassification, without ever touching or deleting the original chain. **This makes the full taxonomic hash chain (WP-45) STABLE** — previously EXPERIMENTAL due to the lack of a re-anchoring tool. Available from the Taxonomy Navigator's taxon detail panel. — Optional PostgreSQL connector behind a new `postgres` Cargo feature (off by default): connectivity testing and a foundation schema bootstrap (specimens, subcultures, audit_log, taxa, strains) via `sqlx`, without changing the live backend — SQLite remains the only database actually serving reads/writes. LAN sync change-detection reuses the existing per-lineage audit hash chain (`lineage_id`, `chain_seq`, `entry_hash`) rather than a parallel change-tracking system: `get_changes_since_cursor` returns entries newer than a peer's cursor, `apply_incoming_changes` classifies each incoming entry as new / duplicate / a genuine fork and durably records any fork in a new `sync_conflicts` table (never silently merged), and `register_sync_peer`/`list_sync_peers` track known devices in a new `sync_peers` table. Migration 035 adds both tables plus a `backend_type` setting. New "Multi-User Backend (Preview)" section in Settings. Networking/discovery and cross-device write-back of accepted changes are explicitly future work. 40 new Rust unit tests. Total: 322 Rust tests.

- **Automated Tests** — Vitest test suite covers utility functions, export row builders, import validation, lab profile store (including domain manifest exports), and the WP-62 offline mutation queue (enqueue ordering, full/partial replay, stop-at-first-failure semantics) — 104 assertions across 5 test files. Rust unit tests (467 test functions with `--no-default-features` as of v1.40.2, up from 463 at v1.40.0, verified via `cargo test --lib --no-default-features`; 499 with the default `tauri-commands` feature as of v1.40.0 — see CHANGELOG.md's per-release entries for the current exact count, since building with `tauri-commands` requires system GTK/WebKit libraries not present in every dev environment) cover accession number generation, pagination helpers, stock depletion rules, all compliance auto-flag SQL rules (including the new mycoplasma rule), **hash-chain invariants** (per-lineage `chain_seq`, fork invariants, `compute_entry_hash` determinism), Merkle checkpoint correctness, death workflow invariants, profile-aware dashboard queries, check_profile_change_allowed confirmation logic, cell_culture vocabulary seeding (expanded stage set, propagation methods, supplement types, biomanufacturing compliance terms), **mycology vocabulary seeding** (stage lifecycle, terminal stages, propagation methods, supplement types, compliance terms, inventory categories, profile isolation, idempotency), **mycology colonization & contaminant tracking** (migration 028 column existence, CHECK constraint, NULL defaults, contaminant-type grouping, empty-when-NULL, recent-events inclusion), **genetic lineage markers** (migration 029 `origin_type` column existence, CHECK constraint acceptance/rejection, `is_best_performer` default and filter), **strain/cultivar data model** invariants (status transitions, hash-chain seeding, hybridization guard), **taxonomy backbone** (taxa table structure, rank constraint, backfill idempotency), **pedigree traversal** (ancestry, descendants, max-depth capping, cycle detection in both directions, specimen tree with/without descendants, export bundle), **PDL/doubling-time calculations** (typical growth, decline, no-growth, and invalid-input cases), **cryopreservation** (location composition, insert, zero-count rejection, thaw decrement, depleted status, PDL/passage inheritance, overdraw rejection, discard, list filtering), **mycoplasma status queries** (active-only filter, last-test-date selection, null-when-no-record, non-mycoplasma exclusion), **cell-culture dashboard queries** (vial summary grouping, depleted/discarded exclusion, maintenance alert thresholds, profile scoping, terminal-stage exclusion, created-at fallback), and **fruiting records** (insert + get round-trip, list per specimen, FK rejection of unknown specimen, migration 030 table/index existence, flush_number DEFAULT), **mycology QC compliance rules** (open_contamination detected/not-raised-for-terminal, overdue_transfer no-subculture/recent-passage-ok, slow_colonization triggered/recent-ok/above-threshold-ok, archived-excluded), **taxon hash chain** (root taxon uses ZERO_HASH, child seeds from parent, update appends correctly, species genesis seeds from genus, strain falls back when no genus entry), **cross-domain taxonomy** (migration 032 domain column existence and per-profile defaults, `active_domain` reads/falls-back correctly), **breeding programs** (migration 033 table/index existence, cascade delete, create/get/list program round-trips, add/list record round-trips, generational summary aggregation, FK rejection for unknown program), and **advanced hybridization** (generation label suggestion rules, backcross detection at direct and grandparent depths, `suggest_generation_label_for_parents` backcross override, per-generation specimen stats), and **multi-user backend + LAN sync foundation** (connection-string validation, backend-switch validation matrix, bootstrap-schema statement splitting and table coverage, feature-off stub error messages, change-vector correctness with single/multiple cursors and limits, all three conflict-classification outcomes, conflict/peer CRUD round-trips, aggregate status counts), and, added in the **v1.39.2 fix pass** following a self-review of WP-50–55: a regression test suite for the genomic-fingerprint masking/write-guard corruption bug (`apply_strain_status_update` — read-masked-then-update preserves the real value; `COALESCE` preserves on omission; normal genomic-confirmation flow unaffected), `FieldPermissionSet` batch-loading correctness, a notification-pipeline masking backstop, SMTP-password backup redaction, and environmental-sensor `source` field validation. Additionally, v1.40.0 added Ed25519 sign/verify/tamper-detection tests (WP-60), Argon2id/AES-256-GCM encryption round-trip and wrong-key/wrong-salt rejection tests (WP-59), plugin manifest validation and vocabulary-isolation tests (WP-61), taxon re-anchoring atomicity/dry-run/verification tests (WP-64), dashboard-cache/cursor-pagination/pedigree-depth tests (WP-63), and Ollama request/response-parsing tests (WP-56) — none requiring a live network service, matching the existing disclosed-limitation pattern for WP-50/WP-52. Both `npm test` and `cargo test` run in CI on every push and block merges on failure (v1.2.4, v1.4.1, v1.6.0, v1.11.0, v1.13.0, v1.14.0, v1.15.0, v1.16.0, v1.17.0, v1.18.0, v1.20.0, v1.21.0, v1.22.0, v1.23.0, v1.24.0, v1.25.0, v1.26.0, v1.27.0, v1.28.0, v1.29.0, v1.30.0, v1.31.0, v1.32.0, v1.33.0, v1.34.0, v1.35.0, v1.36.0, v1.37.0, v1.38.0, v1.39.2, v1.40.0).
- **Accessibility** — WCAG 2.1 AA target: visible `:focus-visible` keyboard indicators, skip-to-content link, ARIA landmarks and `aria-current` on sidebar navigation, focus trapping in QR modal and photo lightbox, `aria-label` on all icon-only buttons, ARIA attributes (`aria-valuenow`/`aria-valuetext`) on health status sliders (v1.2.6). **Label-association pass (v1.40.0, WP-65)** — the 90 remaining `svelte-check` label-association warnings (WCAG 1.3.1, tracked since v1.2.3) are now zero, via a shared `FormField.svelte` wrapper with deterministic IDs; no visual changes.
...
│       │   ├── migrations.rs         # 45 schema migrations
...
| `app_settings`       | Key-value settings store; seeded with auto-checkpoint defaults (v1.10.0); `backend_type` key records the intended future database backend (v1.38.0) |
...
| `sync_peers`         | Known LAN peer devices for the sync foundation; `device_id` unique (v1.38.0) |
| `sync_conflicts`     | Durable record of a local/incoming audit-chain fork at the same `(lineage_id, chain_seq)` — never silently discarded (v1.38.0) |
...
| 035 | v1.38.0 | Added `backend_type` (default `'sqlite'`) to `app_settings`; created `sync_peers` (known LAN devices, unique `device_id`) and `sync_conflicts` (durable fork records) tables for the WP-50/WP-51 multi-user + LAN sync foundation |
...
**SMTP credential redaction (v1.39.2):** the SMTP password (`smtp_config.password`) is stored in plaintext in the *live* database — there is no OS-keychain integration yet. To keep that secret from leaving the machine via a backup file, `create_backup` redacts it (sets it to `NULL`) in the backup copy only; every other SMTP field (host, username, from address) is preserved. Restoring from a backup will require re-entering the SMTP password afterward. The live database is not affected by this redaction.
...
- [x] **Phase TX-2 remaining** — NCBI Taxonomy import + sync with conflict resolution (WP-36, v1.19.0), multi-generational pedigree tools (WP-37, v1.20.0), advanced hybridization (WP-38, v1.21.0), full-rank Taxonomy Navigator with filtering and descendant counts (WP-39, v1.22.0) — **Phase TX-2 complete**
- [x] **SteloCC (Cell Culture)** — cell line registry, passage number / PDL tracking, cryopreservation & LN2 inventory, mycoplasma compliance rules — shipped as the `cell_culture` lab profile (Phase D, WP-30–34, v1.23.0–v1.27.0)
- [x] **SteloMyco (Mycology)** — strain/isolate registry, substrate composition, colonization % and contaminant type tracking, genetic lineage markers, fruiting conditions & yield tracking, mycology QC compliance rules — shipped as the `mycology` lab profile (Phase E, WP-40–44, v1.28.0–v1.32.0)

The six items below were "planned" the last time this list was written; all have since shipped in some form. See the **Phase F — Cross-cutting (complete)** section just below for the accurate, current description of each, including what's still foundation-only:

- [x] **PostgreSQL backend** — optional connector behind the `postgres` Cargo feature; connectivity-tested and unit-tested, but SQLite remains the only backend actually serving reads/writes (WP-50, v1.38.0)
- [x] **Network sync** — LAN sync change-detection and conflict recording on the existing audit hash chain; no network transport or automatic merge yet (WP-51, v1.38.0)
- [x] **Email / push notifications** — desktop + email digest notifications with a background scheduler (WP-52, v1.39.0)
- [x] **iOS support** — Tauri 2 iOS CI scaffold; explicitly unverified end-to-end, no macOS/Xcode/Apple Developer access available (WP-53, v1.39.0)
- [x] **Environmental monitoring integration** — manual sensor-reading entry with sparklines and threshold alerts; USB/BLE/MQTT hardware transport not yet wired (WP-54, v1.39.0)
- [x] **Role-based field-level permissions** — admin-configurable role × field visibility matrix, wired into `strains.genomic_fingerprint` and `breeding_programs.{goal,target_traits}` (WP-55, v1.39.0)

### Phase TX-3 — ✅ Shipped (v1.33.0–v1.37.0)

- [x] **Cross-domain taxonomy** — Plantae/Animalia/Fungi domain manifests with per-domain rank orders and vocabulary labels (v1.34.0)
- [x] **Breeding programs** — structured multi-generational selection tracking with fitness scoring and generational performance analytics (v1.35.0)
- [x] **Advanced hybridization** — F1–F4 / BCn generation labeling, backcross detection, admin-only cross-species override with permanent audit warning (v1.36.0)
- [x] **Custom taxa & Darwin Core export** — provisional taxon management, taxon-to-taxon mappings, DwC JSON export for GBIF/iDigBio (v1.37.0)
- [x] **Full taxonomic hash chain (EXPERIMENTAL)** — Kingdom → Genus → Species → Strain → Specimen cryptographic chain (v1.33.0; see reclassification warning in ROADMAP.md)

### Phase F — Cross-cutting (complete)

- [x] **Multi-user backend + LAN sync foundation** — optional PostgreSQL connector behind the `postgres` Cargo feature (connectivity test + schema bootstrap; not yet wired into the live query layer); LAN sync change-detection and conflict recording built on the existing audit hash chain (`sync_peers`, `sync_conflicts`); no networking/transport layer yet (WP-50, WP-51, v1.38.0)
- [x] **Notifications, iOS scaffold, sensor integration, field-level permissions** (WP-52–55, v1.39.0)
- [x] **Local AI analysis** — Ollama-backed note summarization, passage-comment suggestions, and photo contamination analysis, all draft-and-approve (WP-56, v1.40.0)
- [x] **Interactive lab map** — floor-plan pins and density/contamination/age heat-map (WP-57, v1.40.0)
- [x] **Advanced analytics & reporting** — trend charts, KPI dashboards, exportable multi-sheet reports (WP-58, v1.40.0)
- [x] **Cloud backup with E2E encryption** — Argon2id + AES-256-GCM, `local_nas`/`smb` transport live, S3/SFTP config-only (WP-59, v1.40.0)
- [x] **Regulatory compliance exports** — FDA 21 CFR Part 11 (Ed25519-signed), USDA APHIS permit, CITES dossier (WP-60, v1.40.0)
- [x] **Plugin/extension system** — vocabulary packs + `.steloplugin` format live; WASM compliance-rule execution deferred (WP-61, v1.40.0)
- [x] **Offline-first PWA** — installable web app with Service Worker, IndexedDB offline queue; Tauri-gated so the desktop app is unaffected; no remote mutation endpoint yet (WP-62, v1.40.0)
- [x] **Performance & scalability hardening** — covering indexes, dashboard cache, cursor pagination, virtual scroll, Criterion benches (WP-63, v1.40.0)
- [x] **Taxon chain re-anchoring tool** — makes the WP-45 taxonomic hash chain STABLE (WP-64, v1.40.0)
- [x] **A11y completion pass** — 90 → 0 label-association warnings (WP-65, v1.40.0)

### Beyond v2.x

- [ ] **On-chain anchoring** — publish checkpoint Merkle roots to Dogecoin via `OP_RETURN` for third-party tamper-evidence (WP-66+)
...
- [ ] **WASM compliance-rule execution sandbox** — live execution of plugin-authored compliance rules (deferred from WP-61)
- [ ] **Remote API layer for the PWA** — so a browser-only install can perform mutations, not just reads (deferred from WP-62)
- [ ] **S3/SFTP cloud transport** — live network upload/download for those two backup target types (deferred from WP-59)
- [ ] **Federated networks** — inter-lab specimen passports, shared taxonomy registry (Phase G, WP-70–72)
