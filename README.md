...
- **Settings** — Admin-only Settings view (sidebar gear icon) for configuring lab-wide options. Currently: lab profile switcher (`plant_tissue_culture | cell_culture | mycology`) with a warning banner and mandatory `CHANGE PROFILE` confirmation when data already exists; profile change updates the Svelte store immediately so all components react without restart. Both `plant_tissue_culture` and `cell_culture` profiles have fully seeded vocabulary (stages, propagation methods, hormone types, compliance types, agencies, and inventory categories); the `cell_culture` profile includes an expanded stage set covering common lifecycle states (thawed, adherent, confluent, passaged, cryopreserved, contaminated, discarded) in addition to phase-based stages; `mycology` vocabulary is not yet seeded (v1.14.0, v1.15.0, v1.23.0).
...
- **Automated Tests** — Vitest test suite covers utility functions, export row builders, import validation, and lab profile store — ~107 assertions across 4 test files. Rust unit tests (197 test functions) cover accession number generation, pagination helpers, stock depletion rules, all compliance auto-flag SQL rules (including the new mycoplasma rule), **hash-chain invariants** (per-lineage `chain_seq`, fork invariants, `compute_entry_hash` determinism), Merkle checkpoint correctness, death workflow invariants, profile-aware dashboard queries, check_profile_change_allowed confirmation logic, cell_culture vocabulary seeding (expanded stage set, propagation methods, supplement types, biomanufacturing compliance terms), **strain/cultivar data model** invariants (status transitions, hash-chain seeding, hybridization guard), **taxonomy backbone** (taxa table structure, rank constraint, backfill idempotency), **pedigree traversal** (ancestry, descendants, max-depth capping, cycle detection in both directions, specimen tree with/without descendants, export bundle), **PDL/doubling-time calculations** (typical growth, decline, no-growth, and invalid-input cases), **cryopreservation** (location composition, insert, zero-count rejection, thaw decrement, depleted status, PDL/passage inheritance, overdraw rejection, discard, list filtering), and **mycoplasma status queries** (active-only filter, last-test-date selection, null-when-no-record, non-mycoplasma exclusion). Both `npm test` and `cargo test` run in CI on every push and block merges on failure (v1.2.4, v1.4.1, v1.6.0, v1.11.0, v1.13.0, v1.14.0, v1.15.0, v1.16.0, v1.17.0, v1.18.0, v1.20.0, v1.21.0, v1.22.0, v1.23.0, v1.24.0, v1.25.0, v1.26.0).
...
| `db::migrations` | Migration fixture correctness; cell_culture stage count (12), single terminal stage, propagation method count (7), hormone type count (4), compliance record type count (9), compliance agency count (4), inventory category count (7), isolation from PTC vocabulary (v1.15.0); **taxa backbone** — `taxa` table created with correct columns, rank CHECK constraint enforced, `get_taxon_descendants` returns empty tree for unknown taxon, backfill idempotent, `ncbi_taxon_id` nullable on `taxa` and `species`, `taxon_path` JSON stored correctly (v1.18.0) — 30 tests |
