...
| v1.39.0+ *(Phase F)* | **WP-52 — Email/push notifications** for reminders, overdue items, contamination alerts | future |
| v1.40.0+ *(Phase F)* | **WP-53 — iOS support** (Tauri 2 iOS target, signed IPA, responsive layout) | future |
| v1.41.0+ *(Phase F)* | **WP-54 — Environmental sensor integration** (temp/RH/CO₂, USB/BT/MQTT, readings linked to passages) | future |
| v1.42.0+ *(Phase F)* | **WP-55 — Role-based field-level permissions** (field masking, admin-configurable per role) | future |
| v1.43.0+ *(Phase F)* | **WP-56 — Local AI analysis** (Ollama/MLX integration, note summarization, contamination detection) | future |
| v1.44.0+ *(Phase F)* | **WP-57 — Interactive lab map** (floor-plan upload, specimen pins, contamination heat-map) | future |
| v1.45.0+ *(Phase F)* | **WP-58 — Advanced analytics & reporting dashboards** (trend charts, KPI panels, exportable multi-sheet Excel report) | future |
| v1.46.0+ *(Phase F)* | **WP-59 — Cloud backup & multi-device sync with E2E encryption** (S3/SFTP/NAS targets, AES-256-GCM, offline delta journal sync) | future |
| v1.47.0+ *(Phase F)* | **WP-60 — Regulatory compliance export modules** (FDA 21 CFR Part 11, USDA APHIS permit, CITES dossier) | future |
| v1.48.0+ *(Phase F)* | **WP-61 — Plugin/extension system** (WASM compliance rules, vocabulary packs, dashboard panels, `.steloplugin` format) | future |
| v1.49.0+ *(Phase F)* | **WP-62 — PWA & offline-first mobile experience** (Service Worker, IndexedDB offline queue, installable web app) | future |
| v1.50.0+ *(Phase F)* | **WP-63 — Performance & scalability hardening** (100k+ specimens: virtual scroll, cursor pagination, dashboard materialization, Criterion benchmark suite) | future |
| v1.51.0+ *(Phase F)* | **WP-64 — Taxon chain re-anchoring tool** (makes WP-45 EXPERIMENTAL → STABLE; supervised re-anchor of all descendant chains after taxon reclassification) | future |
| v1.52.0+ *(Phase F)* | **WP-65 — A11y completion pass** (WP-12 follow-up: 85 remaining label-association warnings; `<FormField>` wrapper; WCAG 2.1 AA 1.3.1 full compliance) | future |
