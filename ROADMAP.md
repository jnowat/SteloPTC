...
- **Accession number design decision:** Strain information is **never** encoded in the accession number format (`YYYY-MM-DD-SPECIESCODE-SEQ`). The accession number identifies a culture lineage. Strain is a taxonomic classification and appears as supplemental metadata in QR payloads and reports only. This is enforced by design in `create_specimen` — the accession is generated before any strain assignment is applied.
- **Steps:**
  ...
  4. **`create_hybridization_event` command:** Atomically (in one transaction): creates the hybrid strain record, writes both parents to `strain_parents`, writes the `hybridization_events` record capturing both parent `chain_seq` values at the time of the call, writes the hybrid strain genesis audit entry (seeded from species' current `entry_hash`), and writes a hybridize audit entry. Validates: both parent strains belong to the same `species_id` — rejects with a clear error otherwise (cross-species is reserved for Phase TX-3/WP-48). Cycle detection runs before persisting. Returns the new strain ID.
  5. **Status validation:** `update_strain_status` to `confirmed_manual` is a restricted operation: (a) `confirmation_basis` must be present and non-whitespace — backend rejects with an error if absent; (b) on success the command returns `{ ok: true, warning: "ConfirmedManualWarning" }` (typed enum value, not a free string) — the UI is required to show a blocking acknowledgment modal on this return value (see WP-29). Promoting to `confirmed_genomic` requires `genomic_fingerprint` non-null and non-empty — reject with a clear error otherwise.
  6. **Unit tests:** strain genesis `prev_hash` equals species' current `entry_hash`; strain's `entry_hash` becomes specimen's `prev_hash` when `strain_id` is set; `strain_chain_seq` matches at creation; `update_strain_status` to `confirmed_manual` rejects missing `confirmation_basis`; `update_strain_status` to `confirmed_genomic` rejects null fingerprint; `create_hybridization_event` rejects cross-species parents; `create_hybridization_event` records correct parent chain_seqs; split siblings with a strain still share the same `prev_hash` (fork invariant preserved, `queries.rs` test extended).
...
- **Steps:**
  1. **StrainManager.svelte:** Accessible from the Species detail/management page. Per-species strain list: name, code, type, status badge, specimen count, created date. **Status badges — strict rules, no deviation:**
     - `unverified_claimed` → grey `Claimed` badge.
     - `confirmed_manual` → **always** amber `⚠ Manual ID` badge. The word "Confirmed" must NOT appear without the `⚠` symbol and "Manual" qualifier in any badge, label, or tooltip. This designation is permanent — the badge never upgrades to a clean "confirmed" indicator.
     - `confirmed_genomic` → green `✓ Genomic` badge.
     Actions: create, edit, archive, update status.
