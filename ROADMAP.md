...
> **The plan, the current state, and per-work-packet status.**
> Release-by-release detail lives in [`CHANGELOG.md`](CHANGELOG.md); the contributor
> playbook lives in [`skills.md`](skills.md); the end-user guide in [`UserManual.md`](UserManual.md).
...
| **Tests** | 640 Rust (`--no-default-features`) · **677** with the full `tauri-commands` feature · 113 Vitest · `svelte-check` 0 errors / 0 warnings |
...
| **Open work** | See [Status at a glance](#status-at-a-glance) and [§8a — Beyond Phase F](#8a-beyond-phase-f--long-term-vision--reserved-wp-series) |

**Goal:** one engine, three lab disciplines. Harden and polish the shipped plant-tissue-culture
product, then serve **Cell Culture** and **Mycology** from the same codebase — without forking it
three ways.

**Assets to preserve (don't regress these):** the error-logging system with form-payload capture;
the immutable audit trail and its cryptographic hash-chain / Merkle integrity layer; the
contamination-overview dashboard panel.

**Security baseline (established in Phase A, never relaxed since):** `csp` is a locked-down policy
(`script-src 'self'`, no longer `null` — WP-02), and the default `admin/admin` credential is gated
...
| 049 | `specimen_passports` (issued/imported register) | WP-70 | v1.45.0 |
| 048 | `regulatory_submissions` | WP-68 | v1.44.0 |
| 047 | `user_signing_keys` + `signed_events` | WP-67 | v1.43.0 |
| 046 | `checkpoint_anchors` | WP-66 | v1.42.0 |
| 045 | `installed_plugins` | WP-61 | v1.40.0 |
| 044 | `signing_keys` (Ed25519 export signing) | WP-60 | v1.40.0 |
| 043 | `reanchor_events` | WP-64 | v1.40.0 |
| 042 | `backup_targets` + `cloud_sync_segments` | WP-59 | v1.40.0 |
| 041 | `ai_suggestions` | WP-56 | v1.40.0 |
| 040 | `locations` + `specimens.location_id` | WP-57 | v1.40.0 |
| 039 | Covering indexes for the performance pass | WP-63 | v1.40.0 |
| 038 | `notification_preferences` + `smtp_config`; `notification_check_interval_minutes` | WP-52 | v1.39.0 |
| 037 | `environmental_readings` | WP-54 | v1.39.0 |
| 036 | `field_permissions` | WP-55 | v1.39.0 |
| 035 | `app_settings.backend_type`; `sync_peers` + `sync_conflicts` | WP-50 / WP-51 | v1.38.0 |
| 034 | `taxa.status` / `taxa.provisional_notes`; `taxon_mappings` | WP-49 | v1.37.0 |
| 033 | `breeding_programs` + `breeding_records` (cascade delete, indexes) | WP-47 | v1.35.0 |
| 032 | `app_config.domain` | WP-46 | v1.34.0 |
| 031 | Backfills genesis audit entries for existing taxa | WP-45 (STABLE since v1.40.0) | v1.33.0 |
| 030 | `fruiting_records` | Mycology fruiting/yield | v1.31.0 |
| 029 | `specimens.origin_type` + `specimens.is_best_performer` | Mycology lineage | v1.30.0 |
| 028 | `subcultures.colonization_pct` + `subcultures.contaminant_type` | Mycology colonization | v1.29.0 |
| 027 | Seeds mycology profile vocabulary | WP-40 | v1.28.0 |
| 026 | `specimens.biosafety_level` | WP-33 | v1.26.0 |
| 025 | `frozen_vials` | WP-32 | v1.25.0 |
| 024 | PDL columns on `specimens` / `subcultures` | WP-31 | v1.24.0 |
| 023 | Expands `cell_culture` vocabulary | WP-30 | v1.23.0 |
| 022 | Generation-label + backcross-depth on `hybridization_events`; `strains.is_cross_species` | WP-38 | v1.21.0 |
| 021 | `ncbi_sync_log` | WP-36 | v1.19.0 |
| 020 | `taxa` (Kingdom → Genus hierarchy) | WP-35 | v1.18.0 |
| 019 | Strain/Cultivar data model | WP-28 | v1.16.0 |
| 018 | Seeds `cell_culture` vocabulary | WP-27 | v1.15.0 |
| 017 / 016 | Hardcoded vocabularies → profile-scoped lookup tables; **drops the stage `CHECK` constraint for good** | WP-23 / WP-24 | v1.12.0 |
| 015 | `subcultures.event_type`; `app_config` | WP-22 | v1.11.0 |
| 014 | `app_settings` + auto-checkpoint flags | WP-21 | v1.10.0 |
| 013 | `audit_checkpoints` (Merkle) | WP-20 | v1.9.0 |
| 012 | Contamination columns on `specimens` | — | v1.8.0 |
| 011 | `media_batches.is_draft` | — | v1.8.0 |
| 010 | Generational depth | — | v1.7.0 |
| 009 | Per-lineage hash chain | WP-18 | v1.6.0 |
| 008 | Hash-chain columns on `audit_log` | WP-18 | v1.5.0 |
| 007 | Performance indexes | WP-15 | v1.2.7 |
| 003 / 002 | Rebuilt, then expanded, the stage `CHECK` constraint — the pattern migration 016 finally retired | — | early |

</details>

<details>
<summary><b>Release narrative — how the phases actually landed</b> (click to expand)</summary>

Every release below is documented packet-by-packet in [§10 Versioning plan](#10-versioning-plan)
and line-by-line in [`CHANGELOG.md`](CHANGELOG.md). This is the two-minute version.

- **v0.1.20 – v1.1.0 · Phase A** — security (forced password change, locked CSP), the first signed
  release, crash-proofing, onboarding.
- **v1.1.1 – v1.3.0 · Phase B polish** — bug clearance, work queue, design tokens, a11y, the first
  test harness, backup/restore, Excel import.
- **v1.5.0 – v1.10.0 · Trust Layer Phase 1 (WP-18–21)** — SHA-256 hash-chained audit log, per-lineage
  chains with splits forking from the parent hash, Merkle checkpoints, portable offline proofs.
- **v1.11.0 – v1.15.0 · Phase C** — the keystone refactor: vocabulary moves from `CHECK` constraints
  into profile-scoped lookup tables, making one codebase serve three disciplines.
- **v1.16.0 – v1.22.0 · Phase TX-1/TX-2** — strain/cultivar registry, hybrid wizard, taxonomy
  backbone, NCBI sync, pedigree tools, multi-column navigator.
- **v1.23.0 – v1.27.0 · Phase D** — Cell Culture vertical (PDL, cryo/LN₂, mycoplasma, dashboards).
- **v1.28.0 – v1.32.0 · Phase E** — Mycology vertical (colonization, fruiting/yield, QC rules).
- **v1.33.0 – v1.37.0 · Phase TX-3** — full taxonomic hash chain, cross-domain taxonomy, breeding
  programs, advanced hybridization, custom taxa + Darwin Core export.
- **v1.38.0 – v1.41.0 · Phase F** — PostgreSQL/LAN-sync foundations (v1.38.0), notifications, sensors
  and field permissions (v1.39.0), the WP-56–65 mega-release (v1.40.0: AI, lab map, analytics, cloud
  backup, regulatory exports, plugins, PWA, performance, taxon re-anchoring, a11y), then two
  hardening passes (v1.40.1/.2) and the local-AI runtime + docs overhaul (v1.41.0).
- **v1.42.0 – v1.44.0 · Trust Layer Phase 2/3 + submissions** — Dogecoin `OP_RETURN` anchoring,
  the Ed25519 signed-event ledger, the automated regulatory submission pipeline.
- **v1.45.0 – v1.47.0 · Phase G** — federated inter-lab exchange: specimen passport, shared taxonomy
  registry, cross-lab breeding coordination. Each ships the verifiable core; none bundles a network
  transport.
- **v1.48.0** — WP-73 domain-congruence & security hardening (discharged the `skills.md` §8 backlog).
- **v1.49.0 – v1.53.0 · Phase H** — operational integrity: profile-pluggable compliance rule engine,
  signed lifecycle events across passages and splits, admin data-integrity self-check, compliance
  flag waivers, environmental out-of-range monitoring.
- **v1.53.1** — critical fix pass: Excel round-trip data loss, an AI-command app-wide freeze,
  non-atomic federated imports, ~10 frontend correctness bugs.
...
**Legend:** ✅ shipped & stable · 🧩 shipped, foundation-only (clearly disclosed) · 🧪 experimental / unverified · ⏳ planned

| Phase | Scope | Status |
|---|---|---|
| **A — Ship PTC v1.0** | Security, signed release, crash-proofing | ✅ v0.1.20 → v1.1.0 |
| **B — Polish, stability & Trust Layer** | Design tokens, a11y, tests, hash chain + Merkle checkpoints | ✅ v1.1–v1.10 |
| **C — De-harden the domain** | Vocabulary CHECK constraints → data-driven lookup tables | ✅ v1.11–v1.15 |
| **TX-1/2/3 — Taxonomy & provenance** | Strains, hybridization, pedigree, taxonomy backbone, breeding, Darwin Core | ✅ v1.16–v1.37 |
| **D — Cell Culture vertical** | PDL, cryopreservation, mycoplasma, dashboards | ✅ v1.23–v1.27 |
| **E — Mycology vertical** | Colonization, fruiting/yield, QC rules | ✅ v1.28–v1.32 |
| **F — Cross-cutting** | Notifications, sensors, permissions, AI, lab map, analytics, cloud backup, compliance exports, plugins, PWA, perf, re-anchoring, a11y | ✅ v1.38–v1.41 |
| **Trust Layer 2/3 + submissions** | On-chain (Dogecoin) anchoring, signed-event ledger, auto regulatory-submission pipeline | ✅ v1.42–v1.44 (WP-66–68) |
| **G — Federated networks (complete)** | Signed specimen passport + shared taxonomy registry + cross-lab breeding coordination: independent inter-lab verification, import into the receiver's audit chain, and record-by-record reconciliation | ✅ v1.45–v1.47 (WP-70, WP-71, WP-72) — **Phase G complete** |
| **Domain-congruence & security hardening** | Signed-ledger key-substitution fix, server-side forced-password-change, domain-manifest-driven strain types, Fruiting overview | ✅ v1.48 (WP-73) |
...
| Capability | What works | What's deferred | WP |
|---|---|---|---|
| PostgreSQL backend | Connector compiles + unit-tested | Not wired to the live query layer — SQLite serves all reads/writes | WP-50 |
| LAN sync | Change-detection + conflict recording on the audit hash chain | No network transport, no automatic merge | WP-51 |
| iOS | CI scaffold, Rust compiles for iOS target | Never verified on a real device/simulator (no Mac/Apple Developer access) | WP-53 |
| Environmental sensors | Manual entry, sparklines, out-of-range flags via the compliance engine (WP-78) | USB/BLE/MQTT hardware ingestion | WP-54 |
| Cloud backup targets | `local_nas`/`smb` fully live | S3/SFTP config-only (no network client) | WP-59 |
| Plugin system | Vocabulary packs seed live | WASM compliance-rule execution not yet run | WP-61 |
| PWA | Installable, all read views offline | Data mutations still require the desktop app (no remote API) | WP-62 |
| On-chain anchoring | Prepares the exact Dogecoin `OP_RETURN` payload for a checkpoint root and independently verifies on-chain data against it (trustless) | No automatic broadcast — the transaction is sent with an external wallet the operator controls | WP-66 |

**Still planned (not started):** automatic on-chain broadcast (funded-wallet transport for WP-66), a remote API for the PWA, live S3/SFTP transport, and the plugin WASM execution sandbox. **Phases G and H are complete.** Phase G (WP-70–72) extended the Trust Layer across labs; **Phase H (WP-74–78, v1.49–v1.53)** hardened day-to-day operations — a profile-pluggable compliance rule engine (closing the long-open PTC-only-rules gap), signed lifecycle events across passages and splits, an admin data-integrity self-check, compliance flag waivers, and environmental out-of-range monitoring. Each Phase-G packet ships the verifiable core without bundling a network transport (a networked passport/registry/coordination transport is the long-term follow-up). Full detail in the "Beyond v2.x" and per-packet sections.
...
- **Re-affirmed (v1.40.2 stabilization sprint):** this section was re-read against the code and remains accurate — no PostgreSQL code changed this sprint, no live-server verification occurred (still none available in CI), and SQLite is still the sole backend serving every read and write. The "Multi-User Backend (Preview)" Settings card continues to label the active backend as SQLite and the Postgres path as a preview/intent-only toggle. The description above is neither more nor less true than it was in v1.39.2; it is not upgraded to imply progress that did not happen.
...
- **Re-affirmed (v1.40.2 stabilization sprint):** re-read against the code and still accurate — no sync code changed this sprint, there is still no network transport, no merge, and no conflict-detail UI, and the classification logic has still only ever run inside single-process unit tests, never between two genuinely separate databases. The "Sync Status (Preview)" panel still shows only aggregate counts with its "networking not implemented yet" note. The bottom line is unchanged: two Stelo instances cannot sync with each other today at any level. This entry is deliberately not softened to sound more finished than it is.
...
- **Hardening (v1.40.1):**
  - **Cloud backup now redacts too.** The redaction that v1.39.2 added to the *local* backup path was extracted to a `pub(crate)` `commands::backup::redact_smtp_password_in_backup`, and the WP-59 cloud path (`commands::cloud_backup::cloud_backup`) now applies the identical redaction to the temp DB copy it encrypts. Both backup exit points — local file and encrypted cloud blob — are now guaranteed free of the plaintext SMTP secret before anything leaves the machine; a restored backup (local or cloud) re-prompts for the password exactly as before. Three unit tests cover the redaction (clears only the password, succeeds when no password was set, and — critically — leaves no `-wal` sidecar that could otherwise carry the unredacted value in a WAL-mode file).
- **Keychain assessment (v1.40.2 stabilization sprint):** OS-keychain-backed SMTP credential storage was re-evaluated for this sprint and **deliberately deferred again**, with the trade-off documented here rather than half-implemented. The blockers are structural, not effort: (1) a native keychain (via a `keyring`-style crate) pulls in a **system** dependency on Linux — `libsecret`/Secret Service — which would break the `--no-default-features` unit-test guarantee that all business logic stays buildable without GTK/WebKit/system libs; (2) the mobile targets (Android Keystore, iOS Keychain) are a *separate* API surface from the desktop backends, so "OS keychain" is really four backends, not one; (3) headless/CI and server-style Linux deployments frequently have no Secret Service daemon at all, forcing a plaintext-fallback path that reintroduces exactly today's exposure. The current mitigations — plaintext stored **only** in the local DB, **never** in any backup (local or cloud), a prominent plaintext-storage warning in the Settings SMTP card, and the standing recommendation to use a dedicated least-privilege/app-specific mail credential — are judged the correct risk posture for a patch release. Keychain integration remains a candidate for a future dedicated packet that can adopt WP-59's opt-in, clearly-labelled key-storage pattern; it should not be squeezed into a hardening sprint.
...
- **Strategy decision (v1.40.2 stabilization sprint) — keep the call-site masking model, harden it against drift:** The two candidate long-term directions were weighed explicitly:
  - **Option A — keep per-call-site masking (chosen).** Each read command masks the sensitive fields it returns via `apply_field_permissions`. This matches how *every* cross-cutting concern in the codebase is applied (audit logging, profile-gating) — explicitly at the call site — and is the idiomatic choice in a language with no runtime reflection.
  - **Option B — response sanitization / typed masking.** A generic interceptor that walks the outgoing response and masks by field name. Rejected: in Rust this needs either a proc-macro/derive framework (a large new abstraction for a 3-field surface) or a serde-value walker that masks by bare field name — which cannot consult the role/entity context masking actually depends on, and would mask unrelated fields that merely share a name (`notes`, `goal`, …). It trades an auditable handful of explicit call sites for an opaque, over-broad mechanism. Not worth it at this surface size, and reconsiderable only if the masked-field set grows by an order of magnitude.
  - **The one real weakness of Option A is *drift*:** a maskable field could gain a `field_permissions` seed row (or a new read path) without a matching mask call, silently leaving data exposed while the admin UI implies it's protected. v1.40.2 pins that shut with three low-effort, zero-behavior-change (for legitimate use) guards, all in `db::permissions`:
    1. **`MASKABLE_FIELDS`** — one canonical constant listing the exact `(entity_type, field_name)` pairs the read path masks. Single source of truth.
    2. **A tripwire test** (`maskable_fields_registry_matches_migration_seed`) that fails the build if the migration seed and `MASKABLE_FIELDS` ever disagree — catching "seeded a field but forgot the call site" (and the reverse) before it ships.
    3. **A guard in `set_field_permission`** that refuses to persist a visibility rule for any field *not* in `MASKABLE_FIELDS`. Previously an admin (or a crafted `invoke`) could store a rule for, say, `strain.name` — a persisted row the read path never consults, i.e. a masking that silently does nothing. It's now rejected loudly. Legitimate use is unaffected (the migration seeds, and the editor only offers, exactly the `MASKABLE_FIELDS` set).
  - **Coverage audit performed this sprint (no bypass found):** `genomic_fingerprint` is carried *only* by the full `Strain` struct, and its two read paths (`get_strain`, `list_strains_by_species`) both mask it; the pedigree/tree/export payloads use `StrainSummary`, which does not carry the field at all, so those paths cannot leak it. `breeding_program.goal`/`target_traits` are masked on both read paths (`get_breeding_program`, `list_breeding_programs`) with a `reject_if_restricted_marker` write-guard on create. The masked surface is fully covered.
  - **Direction unchanged:** extending masking to more entities remains a mechanical follow-up (add a `MASKABLE_FIELDS` entry + a seed row + one `mask_optional_field` call per read site, plus a `reject_if_restricted_marker` guard on every write path for that field), still gated behind the corruption-guard precondition documented above.
...
- **WP-70 — Federated identity & inter-lab specimen transfer:** Define a signed "specimen passport" format that a receiving lab can verify independently of the originating lab's database. Receiving labs import the passport into their own audit chain. No central authority required. — ✅ **Delivered in v1.45.0.** As built: a signed, self-contained JSON passport (`src-tauri/src/passport/`) carrying the issuer identity (lab name + Ed25519 public key), the specimen identity subset, and its provenance as canonical audit entries — so a receiver recomputes every entry hash, the chain linkage, and (if a checkpoint sealed exactly those entries) the Merkle root, then checks the issuer's Ed25519 signature over the content hash. Independent by construction: verifiable with only the issuer's public key and the embedded chain (a ~40-line standalone Python verifier ships in `docs/specimen-passport.md`). Signing reuses the WP-60 lab key; migration 049 adds `specimen_passports` (issued/imported register, `UNIQUE(direction, passport_id)`). `import_specimen_passport` refuses an unverifiable or duplicate passport and folds an accepted one into the receiver's own hash-chained audit log (a `passport_imported` entry committing to the content hash). Chose a compact deterministic canonical byte form over JSON-LD (no external `@context`/RDF dependency; the standalone verifier stays trivial). UI: Audit Log → Specimen Passports panel + an Issue Passport action on specimen detail. 21 Rust unit tests. **Deferred, disclosed honestly (WP-66 precedent):** no network transport — issuing downloads a file, importing reads one; the operator moves it via their own channel.
- **WP-70 chose a compact canonical form over JSON-LD** — the original sketch said "JSON-LD + Merkle proof". JSON-LD would pull in an `@context`/RDF-normalization dependency and complicate the standalone verifier for no gain here, since both labs run the same schema; the passport uses the same deterministic pipe/control-char canonical form as the rest of the Trust Layer, and carries the Merkle anchor optionally rather than always. Documented in `docs/specimen-passport.md`.
- **WP-71 — Shared taxonomy registry:** A lightweight read-only taxonomy server that multiple Stelo installations can subscribe to for shared genus/species/strain records. Strain records carry the originating lab's cryptographic signature. Labs can accept, override, or fork entries. — ✅ **Delivered in v1.46.0.** As built: a signed, self-contained JSON **taxonomy registry** (`src-tauri/src/registry/`) carrying the issuer identity (lab name + Ed25519 public key) and this lab's genus-and-above `taxa`, `species`, and non-archived `strains` as flat records keyed by a **name-based, cross-lab-stable natural key** (`taxon|genus|Citrus`, `species|Citrus sinensis`, `strain|Citrus sinensis|VAL-EARLY` — never a local UUID/`taxon_path`) and sorted by that key so re-exports are byte-identical. A receiver recomputes each record's `record_hash` and the registry's `content_hash` (same `0x1f`/`0x1e` control-char canonical form as WP-70), then checks the issuer's Ed25519 signature — independent by construction, with a ~40-line standalone Python verifier in `docs/taxonomy-registry.md`. Signing reuses the WP-60 lab key (the same key WP-70 uses). Migration 050 adds `taxonomy_registries` (issued/imported register, `UNIQUE(direction, registry_id)`) and `registry_record_dispositions` (per-record merge decisions). Import is **additive/non-destructive** — `preview_import` classifies each incoming record `new`/`identical`/`conflict`, and `import_registry` applies the operator's per-record **accept / override / fork** choice (accept inserts a `new` record; override keeps local; fork inserts a marked divergent copy), folding the whole merge into the receiver's own hash-chained audit log (a `registry_imported` entry committing to the content hash). **Two invariants held and disclosed:** an imported strain is always `unverified` (a foreign `confirmed_genomic` claim is preserved as context in `confirmation_basis` but never inherited), and the genomic fingerprint is never exported. UI: Audit Log → **Shared Taxonomy Registry** panel (export, preview with a per-record disposition picker, import, register). 25 Rust unit tests. **Deferred, disclosed honestly (WP-66/WP-70 precedent):** no subscription server / network transport — exporting downloads a file, importing reads one; the operator moves it via their own channel.
- **WP-72 — Cross-lab breeding program coordination:** Extensions to the breeding program system (WP-47) supporting collaborating labs that maintain separate lineages of the same program and wish to merge their selection records periodically. — ✅ **Delivered in v1.47.0 — Phase G complete.** As built: a signed, self-contained JSON **coordination bundle** (`src-tauri/src/coordination/`) carrying the issuer identity (lab name + Ed25519 public key), one breeding program's header, and its selection records (WP-47 `breeding_records`) as flat records. The program is matched across labs by a **name-based natural key** (its name, never a local UUID) and each selection record references its strain by **scientific name + code** (the same cross-lab identity WP-71 uses) and is keyed by a name-based `source_key` (`sel|<program>|<Genus species> <code>|g<gen>|<date>|<selector>|<content8>`, where `content8` is a short content digest so distinct same-day selections don't collide while byte-identical ones from two labs merge to one) and sorted by that key so re-exports are byte-identical. A receiver recomputes each record's `record_hash` and the bundle's `content_hash` (same `0x1f`/`0x1e` control-char canonical form as WP-70/WP-71), then checks the issuer's Ed25519 signature — independent by construction, with a ~40-line standalone Python verifier in `docs/breeding-coordination.md`. Signing reuses the WP-60 lab key (the same key WP-70/WP-71 use). Migration 051 adds `breeding_bundles` (issued/imported register, `UNIQUE(direction, bundle_id)`), `breeding_bundle_dispositions` (per-record decisions), and an `origin_lab` column on `breeding_records` (provenance of merged records). Merging is **additive/non-destructive** — `preview_import` classifies each incoming record `new`/`identical`/`blocked`, and `import_bundle` applies the operator's per-record **accept / skip** choice, folding the whole merge into the receiver's own hash-chained audit log (a `breeding_merge_imported` entry committing to the content hash). An absent local program is created as a *coordinated copy* (a shell) so records can attach; an existing one is matched by name and never has its metadata overwritten. **Invariants held and disclosed:** merging is a **set union with two dispositions (accept/skip), not three** — a selection record has no local counterpart to WP-71's "override" and forking a log entry is meaningless (documented divergence); a record whose strain is not present locally is `blocked` until the strain is shared via the WP-71 registry (the `breeding_records.strain_id` FK made explicit); and a merged record's `origin_lab` is preserved so a re-export never reattributes a partner's selections. UI: Audit Log → **Cross-Lab Breeding Coordination** panel (pick a program, export, preview with a per-record accept/skip picker, import, register). 28 Rust unit tests. **Deferred, disclosed honestly (WP-66/WP-70/WP-71 precedent):** no coordination server / network transport — exporting downloads a file, importing reads one; the operator moves it via their own channel. Backend test suite was **602** at this release (`--no-default-features`).

### Trust Layer Phase 2 — On-chain anchoring (Dogecoin first) — ✅ Delivered in **v1.42.0**
...
- **As built (v1.42.0):** The deterministic, dependency-free core ships. `anchoring::mod` builds the exact `OP_RETURN` scriptPubKey (`0x6a` + 37-byte pushdata: `STEL` marker + version + 32-byte root; 39 bytes total, under Dogecoin's 80-byte relay limit) and `op_return_matches_root` verifies raw on-chain data against a checkpoint's stored root **trusting only those two inputs** — an auditor can reproduce it with a block explorer and the lab's published root alone (standalone Python recipe in `docs/on-chain-anchoring.md`). Migration 046 adds `checkpoint_anchors` (`prepared` → `submitted` → `confirmed`); recording a broadcast txid writes it back to `audit_checkpoints.anchored_txid` exactly as reserved. `anchoring::store` is a pure, in-memory-testable prepare/record/verify lifecycle; `commands/anchoring.rs` is a thin manage-gated wrapper; the UI is the **Audit Log → Checkpoints → On-Chain Anchoring** panel. 23 Rust unit tests. **Deferred, disclosed honestly:** SteloPTC does not broadcast the transaction — publishing an `OP_RETURN` needs a funded wallet and a node/broadcast API (private keys + money), so that step is done with an external wallet the operator already controls. The trust guarantee is independent of who broadcasts, so the verifiable core ships now (the same "foundation now, credential-bearing transport later" boundary as WP-50/59/61). No new dependency, no network client.

### Trust Layer Phase 3 — Specimen events as signed transactions — ✅ Delivered in **v1.43.0**
...
- **As built (v1.43.0):** The full signing/verification engine ships. Migration 047 adds `user_signing_keys` (one Ed25519 keypair per user — attribution to the *individual*, distinct from the lab-wide WP-60 export key) and `signed_events`, a monotonic (`seq` gapless + UNIQUE), hash-chained ledger where each entry carries a detached Ed25519 signature over its `event_hash`. `signed_ledger::append_signed_event` chains + signs; `verify_ledger` detects content edits, deletions (seq gap), broken linkage, forged signatures, and swapped-key forgery, each with the failing `seq`. Signing reuses the WP-60 `compliance_export::signing` module — no new crypto surface. Commands `record_signed_event`/`list_signed_events`/`verify_signed_event_ledger`/`get_user_signing_public_key`; UI is the **Audit Log → Signed Event Ledger** panel; `docs/signed-event-ledger.md`. 9 Rust unit tests. **One wired demonstrating integration:** every newly created specimen auto-records a signed `specimen_created` genesis transaction (best-effort — never fails the create). **Deferred, disclosed honestly:** extending *automatic* signing to all ~30 mutation commands is incremental (each a one-line `try_append_signed_event(...)`; `record_signed_event` already signs any event today) — the same "exhaustive command-layer sweep is disproportionate" boundary as WP-63.

### Regulatory submission pipeline (advanced) — ✅ Delivered in **v1.44.0**
...
- **As built (v1.44.0):** The monitoring + readiness + generation pipeline ships. Migration 048 adds `regulatory_submissions` (`ready`/`blocked` → `generated` → `submitted` → `acknowledged`). `reg_submission::evaluate_readiness` runs kind-specific read-only checks (Part 11: date range + audit entries + chain verified + users; USDA: PTC profile + specimens exist/named + no expired permits; CITES: root specimen + Appendix + chain verified). `commands::reg_submission` generates the WP-60 bundle for a `ready` submission, signs + zips it (reusing `compliance_export::{signing, zip_writer}`), adds a top-level Ed25519 signature over the `.zip`, and writes it under `compliance_exports/submissions/`. `run_submission_monitor` — also wired into the background scheduler on each tick — re-evaluates all non-terminal submissions and auto-generates the ready+`auto_generate` ones. `SubmissionPipelinePanel.svelte` in Compliance; docs in `docs/regulatory-exports.md`. 10 Rust unit tests. **Deferred, disclosed honestly:** SteloPTC produces a signed, ready-to-submit package but does NOT electronically submit to a government portal — that needs authenticated portal credentials, per-agency form APIs, and jurisdiction-specific legal authorization (the same boundary WP-60 already draws). The operator submits through the official channel and records the returned reference in the pipeline.

### Phase H — Operational integrity & compliance hardening (WP-74–78) — ✅ Complete (v1.49.0–v1.53.0)

Phase H turned inward: rather than adding a new domain or a new federation surface, it hardened the day-to-day compliance and integrity machinery the three profiles already share. Each packet's substance lives in a **pure, non-gated module** (compiling and unit-testing under `--no-default-features`) with a thin command wrapper, matching the established `store.rs`/`commands/*.rs` split.

- **WP-74 — Profile-pluggable compliance rule engine.** Discharges the long-open "compliance rule engine is PTC-only / not profile-gated" item (skills.md §8, the WP-25 deviation). Historically `get_compliance_flags` was a flat list of SQL blocks, and the **citrus HLB rule** (`species_code LIKE 'CIT-%'`) ran for *every* profile — so a mycology or cell-culture lab that coded a species `CIT-*` got a spurious plant-quarantine flag it could never clear. As built: a pure `compliance_rules` catalogue where each `RuleDef` declares its `RuleScope` (all profiles vs a named set), and the command evaluates a rule only when `is_rule_active(flag_type, &profile)`. The mycology/cell-culture rule groups (already gated at the call site) now route through the same catalogue. A `list_compliance_rules` command drives an "Active checks for this profile" chip row. No schema change; 9 Rust tests.
- **WP-75 — Signed lifecycle events across mutations.** The WP-67 follow-up (which shipped the signing engine and wired only `specimen_created`). As built: a pure `signed_ledger::lifecycle` submodule centralizing the event-type vocabulary and payload construction, plus best-effort `try_append_signed_event` calls in `create_subculture` (passage / death) and `split_specimen` (split). A ledger hiccup never fails the primary mutation (mirrors `log_audit(...).ok()`). Extending signing to the remaining ~25 mutation commands is the same incremental one-line-per-site follow-up disclosed in WP-67. No schema change; 6 Rust tests.
- **WP-76 — Lab data-integrity self-check.** A provenance app depends on referential soundness, but SQLite doesn't retroactively catch rows that predate a constraint, arrive via import, or survive an out-of-band edit — and never detects an audit-chain gap (a deleted history row). As built: a pure `integrity` module running a battery of read-only checks (specimens → missing species/strain/parent, subcultures → missing specimen/media, strains → missing species, duplicate accessions, and audit-lineage gaps via `COUNT(*) ≠ MAX(chain_seq) + 1`) and returning a structured, severity-sorted report with example ids. Admin-gated `run_data_integrity_check`; `DataIntegrityPanel.svelte` in the Audit Log. No schema change; 7 Rust tests.
- **WP-77 — Compliance flag waivers.** A flag is computed, so a known-and-accepted one reappears on every evaluation. As built: migration **052** `compliance_flag_waivers` (with a `revoked` tombstone kept for the trail); the pure `compliance_rules::is_waived` filters actively-waived `(flag_type, specimen)` pairs out of `get_compliance_flags`; `waive_compliance_flag` (reason mandatory, audit-logged) / `list_compliance_waivers` / `revoke_compliance_waiver`; a per-flag **Waive** action and active-waivers list in the Compliance view. 5 Rust tests.
- **WP-78 — Environmental out-of-range monitoring.** Closes the evaluation gap left by the WP-54 sensor foundation (which stored readings but never checked them). As built: a pure `monitoring` module with per-type acceptable ranges (temperature 18–28 °C, humidity 40–95 %, CO₂ 300–5000 ppm, light 500–15000 lux, pH 5.4–6.2; `custom`/unknown never flag) and an `evaluate`/`range_message` API — delivered as a new `environmental_out_of_range` rule in the WP-74 engine, so it inherits the flag UI, the active-rules chip, and the WP-77 waiver workflow with **no new command, table, or migration**. The default ranges are hardcoded (user-configurable thresholds are a disclosed follow-up). 5 Rust tests.
...
| **v1.40.2** *(Phase F stabilization)* | **Android CI fix + masking-model hardening** (no new features): pinned the Android build's Tauri CLI to `--version 2.11.3` to match the linked `tauri` runtime crate (unpinned CLI installs were regenerating `gen/android` from newer templates and breaking the APK-collection step); hardened APK collection to search the full `outputs/` tree with a diagnostic fallback on miss; added a `MASKABLE_FIELDS` registry + tripwire test + `set_field_permission` guard closing the one way a maskable field could gain a read path without a matching mask call; re-verified SMTP redaction on both local and cloud backup paths; re-affirmed WP-50/51 honesty disclosures unchanged. 467 tests (`--no-default-features`, up from 464) | ✅ shipped |
| **v1.41.0** *(Phase F)* | **WP-56b — Local AI runtime hardening + documentation overhaul:** second local AI runtime (LocalAI / OpenAI-compatible via `/v1/chat/completions` + `/v1/models`, alongside the default Ollama); `list_models` + `get_ai_status` reachability probe; classified/actionable errors (dead endpoint, model-not-pulled → exact `ollama pull` hint); new **Settings → AI Assistant** panel (`AiSettingsPanel.svelte`) with live "Test Connection"; README rewritten as a consumer front door; new `docs/local-ai.md`; UserManual expanded to cover Phase F. No schema change (reuses `app_settings`). 479 tests (`--no-default-features`, up from 467) | ✅ shipped |
| **v1.42.0** *(Trust Layer Phase 2)* | **WP-66 — On-chain anchoring (Dogecoin `OP_RETURN`):** migration 046 `checkpoint_anchors`; dependency-free `anchoring::mod` payload/script builder + trustless `op_return_matches_root` verifier; `anchoring::store` prepare/record/verify lifecycle writing back to `audit_checkpoints.anchored_txid`; `commands/anchoring.rs`; `OnChainAnchorPanel.svelte` in the Audit Log checkpoint panel; `docs/on-chain-anchoring.md`; 23 Rust tests. Broadcast deferred to an external wallet (disclosed) | ✅ shipped |
| **v1.43.0** *(Trust Layer Phase 3)* | **WP-67 — Specimen events as signed transactions:** migration 047 `user_signing_keys` + `signed_events`; `signed_ledger` engine (per-user Ed25519 keys, hash-chained + signed ledger, four-invariant `verify_ledger`) reusing the WP-60 signing module; `commands/signed_events.rs`; best-effort signed genesis wired into specimen creation; `SignedLedgerPanel.svelte`; `docs/signed-event-ledger.md`; 9 Rust tests. Auto-signing the remaining mutation commands deferred (disclosed) | ✅ shipped |
| **v1.44.0** *(Phase F+)* | **WP-68 — Regulatory submission pipeline:** migration 048 `regulatory_submissions`; `reg_submission` readiness engine (Part 11 / USDA / CITES checks) + lifecycle state machine; `commands/reg_submission.rs` generates + signs the WP-60 bundle for ready submissions (top-level Ed25519 signature over the `.zip`); `run_submission_monitor` wired into the background scheduler for auto-generation; `SubmissionPipelinePanel.svelte`; docs in `docs/regulatory-exports.md`; 10 Rust tests. Live portal submission deferred to the official channel (disclosed) | ✅ shipped |
| **v1.45.0** *(Phase G)* | **WP-70 — Federated identity & inter-lab specimen transfer (specimen passport):** `src-tauri/src/passport/` (pure `mod.rs` document + 5-check independent verification, connection-level `store.rs` issue/verify/import lifecycle); migration 049 `specimen_passports` (issued/imported, `UNIQUE(direction, passport_id)`); signing reuses the WP-60 lab key (lifted to non-gated `compliance_export::load_or_create_lab_signing_key`); `commands/passport.rs` (7 gated commands); `SpecimenPassportPanel.svelte` in Audit Log + Issue Passport action on specimen detail; standalone Python verifier in `docs/specimen-passport.md`; 21 Rust tests (549 total). No network transport — issuing downloads JSON, importing reads it (disclosed) | ✅ shipped |
| **v1.46.0** *(Phase G)* | **WP-71 — Shared taxonomy registry (federated, signed reference-data exchange):** `src-tauri/src/registry/` (pure `mod.rs` document + 4-check independent verification, connection-level `store.rs` export/preview/import lifecycle with per-record accept/override/fork); migration 050 `taxonomy_registries` + `registry_record_dispositions`; signing reuses the WP-60 lab key; `commands/registry.rs` (7 gated commands); `TaxonomyRegistryPanel.svelte` in Audit Log; standalone Python verifier in `docs/taxonomy-registry.md`; 25 Rust tests (574 total). Import is additive/non-destructive; imported strains are always `unverified`; no network transport (disclosed) | ✅ shipped |
| **v1.47.0** *(Phase G)* | **WP-72 — Cross-lab breeding program coordination (federated, signed selection-log merge):** `src-tauri/src/coordination/` (pure `mod.rs` document + 4-check independent verification, connection-level `store.rs` export/preview/import lifecycle with per-record accept/skip, program-shell creation, strain resolution by scientific name + code); migration 051 `breeding_bundles` + `breeding_bundle_dispositions` + `breeding_records.origin_lab`; signing reuses the WP-60 lab key; `commands/coordination.rs` (7 gated commands); `BreedingCoordinationPanel.svelte` in Audit Log; standalone Python verifier in `docs/breeding-coordination.md`; 28 Rust tests (602 total). Merging is additive/non-destructive (set union, accept/skip); a record whose strain is absent locally is `blocked`; no network transport (disclosed). **Phase G complete** | ✅ shipped |
| **v1.48.0** | **WP-73 — Domain-congruence & security hardening:** discharges the `skills.md` §8 backlog. Security: `signed_ledger::verify_ledger` now fails on a missing registered key (closes a key-substitution forgery), and `validate_session` enforces `must_change_password` server-side (default-deny; `validate_session_allow_password_change` carve-out for `change_password`/`get_current_user`). Domain-UI: `StrainManager` derives strain types from the domain manifest (first real manifest consumer); `origin_type`/`contaminant_type` single-sourced in `profile.ts`; Cell Count/PDL fields cell-culture-gated; nav gained a `profiles` filter (Media → PTC). New: a top-level **Fruiting** overview (Mycology) backed by `list_all_fruiting_records`. No schema change. +6 Rust / +7 TS tests (608 / 113 total) | ✅ shipped |
| **v1.49.0** *(Phase H)* | **WP-74 — Profile-pluggable compliance rule engine:** new pure `src-tauri/src/compliance_rules/` catalogue (each rule declares which profiles it applies to); `get_compliance_flags` gates every rule via `is_rule_active`, so the citrus HLB rule (`CIT-%`) no longer fires in mycology/cell-culture labs; `list_compliance_rules` command + "Active checks" chip row in the Compliance view. Closes the skills.md §8 "PTC-only rules" item (the WP-25 deviation). No schema change. +9 Rust tests | ✅ shipped |
| **v1.50.0** *(Phase H)* | **WP-75 — Signed lifecycle events across mutations:** the WP-67 follow-up. New pure `signed_ledger::lifecycle` submodule (event-type vocabulary + payload builders); `create_subculture` (passage) and `split_specimen` (split) now append a signed, hash-chained, user-attributed ledger entry (best-effort). No schema change. +6 Rust tests | ✅ shipped |
| **v1.51.0** *(Phase H)* | **WP-76 — Lab data-integrity self-check:** new pure `src-tauri/src/integrity/` module (orphaned specimens/subcultures/strains, broken parent links, duplicate accessions, audit-lineage sequence gaps); admin-gated `run_data_integrity_check` command; `DataIntegrityPanel.svelte` in Audit Log. No schema change. +7 Rust tests | ✅ shipped |
| **v1.52.0** *(Phase H)* | **WP-77 — Compliance flag waivers:** migration **052** `compliance_flag_waivers`; pure `compliance_rules::is_waived` filters waived flags out of `get_compliance_flags`; `waive_compliance_flag`/`list_compliance_waivers`/`revoke_compliance_waiver` commands (reason mandatory, audit-logged); per-flag **Waive** action + active-waivers list in the Compliance view. +5 Rust tests | ✅ shipped |
| **v1.53.0** *(Phase H)* | **WP-78 — Environmental out-of-range monitoring:** new pure `src-tauri/src/monitoring/` module (per-type acceptable ranges + evaluation); delivered as the `environmental_out_of_range` rule in the WP-74 engine, so it reuses the flag UI and WP-77 waivers with no new command/table/migration. **Phase H complete.** +5 Rust tests | ✅ shipped |
| **v1.53.1** | **Critical fix pass:** Excel round-trip data loss (compliance permit number + media basal salts columns), AI-command app-wide freeze (DB mutex held across the Ollama network call, all 4 commands), non-atomic federated imports (registry/coordination/passport now transactional), and ~10 frontend correctness bugs (broken Excel export, stuck error-log pagination, dropped zero-valued measurements, dead media solid-reagent path + mg/L→g/L label, wrong strain quick-panel results, empty pedigree tab, discarded hybrid strain type, inflated print passage count, print-summary page mismatch, non-reactive photo cache). Backend **640** Rust tests, frontend **113** | ✅ shipped |
| **v1.53.2** | **Build fix, dependency maintenance & documentation pass:** restored a `master` that had been red across every merge gate since v1.53.1 — `commands/subcultures.rs` passed an `i32` where `signed_ledger::lifecycle::passage` takes an `i64`, in code only the full `tauri-commands` build compiles (verified here with a real full-feature run: **677** Rust tests + clippy clean). Closed two high-severity npm advisories (`fast-uri`, `postcss`) and brought four in-range-drifted packages current, lockfile-only. Restructured the ROADMAP header, brought `UserManual.md` from v1.45.0 to current with six new sections (Phase G + H), added `docs/README.md` and uniform `docs/*.md` headers, and added `skills.md` §10 (docs-drift checklist) + the full-feature verification procedure. No schema change | ✅ shipped |
| v2.x+ *(Phase H+)* | Automatic on-chain broadcast (funded-wallet transport for WP-66); automatic signing of *all* mutation commands (WP-75 continuation); live electronic portal submission (WP-68 follow-up); a networked passport/registry/coordination transport (WP-70/WP-71/WP-72 follow-up); live S3/SFTP transport (WP-59); plugin WASM execution sandbox (WP-61); user-configurable compliance-rule thresholds (WP-74/WP-78 follow-up) | long-term |
...
**640** Rust tests with `--no-default-features` and **677** with the full `tauri-commands` feature ·
...
[§10 Versioning plan](#10-versioning-plan) and [`CHANGELOG.md`](CHANGELOG.md).

*Hand packets to Claude Code in order; each is scoped to stand alone.*
