...
- **RECLASSIFICATION WARNING — known limitation (no re-anchoring tool yet):**
  ...
  **Phase F follow-up (WP-64):** A supervised `reanchor_taxon_chain` command is planned for Phase F (WP-64) that will walk all descendant chains after a taxon reclassification, write new genesis entries anchored to the post-reclassification parent state, and record the event in a `reanchor_events` audit table. Once WP-64 ships, this feature's status will be downgraded from **EXPERIMENTAL** to **STABLE**.
...
### WP-50 — PostgreSQL backend option — ✅ Foundation delivered in **v1.38.0**
...
- **Not yet implemented (deferred to a future WP):** the `DbBackend` trait refactor of `db::queries` (~5,800 lines) and all 24 command modules to actually target either backend; PostgreSQL connection pooling held in `AppState`; a schema migration runner that ports all 34 SQLite migrations; encrypted storage of a saved connection string; the "Migrate to PostgreSQL" one-click export/import action. SQLite is the only backend serving live reads/writes — `backend_type` records intent for this future work, not a live toggle.

### WP-51 — LAN network sync across desktop + mobile clients — ✅ Foundation delivered in **v1.38.0**
...
- **As built (v1.38.0) — implemented together with WP-50 for architectural coherence:**
  ...
  - **Commands** (`commands/sync.rs`): `get_sync_status` (any user), `get_changes_since_cursor` (supervisor+ — the read side of the future transport's "what's new since I last synced?" request), `apply_incoming_changes` (admin — detects and durably records conflicts/duplicates), `list_sync_conflicts`/`resolve_sync_conflict` (admin closes a conflict record administratively; does not itself reconcile the data divergence, which requires a human decision about which side is authoritative), `register_sync_peer`/`list_sync_peers`.
...
### WP-52 — Email/push notifications for reminders and overdue items — ✅ Delivered in **v1.39.0**
...
- **Bump:** minor → **v1.39.0**.
- **Status: EXPLICITLY UNVERIFIED.** This packet was implemented in a Linux sandbox with no macOS, no Xcode, and no Apple Developer account available — none of the acceptance criteria above could be exercised. What follows is a disclosed best-effort scaffold, not a working iOS build.
- **As built (v1.39.0):**
  ...
  - **`.github/workflows/build-ios.yml`** (new) — modeled structurally on `build-android.yml`: an unsigned iOS Simulator build job runs on every push/PR (proves the Rust/Swift/Xcode project compiles without needing any signing secrets), and a signed release-IPA job runs on GitHub Release events, gated on five new secrets (`APPLE_CERTIFICATE_BASE64`, `APPLE_CERTIFICATE_PASSWORD`, `APPLE_PROVISIONING_PROFILE_BASE64`, `APPLE_TEAM_ID`, `APPLE_SIGNING_IDENTITY`) that do not exist yet.
...
- **Not yet implemented / not verified (all require a macOS maintainer):**
  1. Running `cargo tauri ios init` even once against this codebase to confirm the generated Xcode project (`src-tauri/gen/apple/`) builds at all.
  2. Generating and adding the five Apple signing secrets listed above.
...
### WP-54 — Environmental sensor integration — ✅ Delivered in **v1.39.0**
...
### WP-55 — Field-level permissions for shared lab use
...
| v1.40.0+ *(Phase F)* | **WP-56 — Local AI analysis** (Ollama/MLX integration, note summarization, contamination detection) | future |
| v1.41.0+ *(Phase F)* | **WP-57 — Interactive lab map** (floor-plan upload, specimen pins, contamination heat-map) | future |
| v1.42.0+ *(Phase F)* | **WP-58 — Advanced analytics & reporting dashboards** (trend charts, KPI panels, exportable multi-sheet Excel report) | future |
| v1.43.0+ *(Phase F)* | **WP-59 — Cloud backup & multi-device sync with E2E encryption** (S3/SFTP/NAS targets, AES-256-GCM, offline delta journal sync) | future |
| v1.44.0+ *(Phase F)* | **WP-60 — Regulatory compliance export modules** (FDA 21 CFR Part 11, USDA APHIS permit, CITES dossier) | future |
| v1.45.0+ *(Phase F)* | **WP-61 — Plugin/extension system** (WASM compliance rules, vocabulary packs, dashboard panels, `.steloplugin` format) | future |
| v1.46.0+ *(Phase F)* | **WP-62 — PWA & offline-first mobile experience** (Service Worker, IndexedDB offline queue, installable web app) | future |
| v1.47.0+ *(Phase F)* | **WP-63 — Performance & scalability hardening** (100k+ specimens: virtual scroll, cursor pagination, dashboard materialization, Criterion benchmark suite) | future |
| v1.48.0+ *(Phase F)* | **WP-64 — Taxon chain re-anchoring tool** (makes WP-45 EXPERIMENTAL → STABLE; supervised re-anchor of all descendant chains after taxon reclassification) | future |
| v1.49.0+ *(Phase F)* | **WP-65 — A11y completion pass** (WP-12 follow-up: 85 remaining label-association warnings; `<FormField>` wrapper; WCAG 2.1 AA 1.3.1 full compliance) | future |
